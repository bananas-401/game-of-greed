...
class GeniusBot(BaseBot):
    def _roll_bank_or_quit(self):
        if self.dice_remaining == 0:
            return "r"

        if self.dice_remaining >= 3 and self.unbanked_points > 1000:
            return "b"

        if self.dice_remaining >= 2 and self.unbanked_points > 500:
            return "b"

        if self.dice_remaining >= 3 and self.unbanked_points > 600:
            return "b"
        else:
            return "r"
